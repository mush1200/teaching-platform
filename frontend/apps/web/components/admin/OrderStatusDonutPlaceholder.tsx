...
export function OrderStatusDonutPlaceholder({ segments }: Props) {
  return (
    <div className="rounded-3xl border border-[#E5E7EB]/80 bg-white p-6 shadow-sm">
      ...
      <p className="mt-1 text-xs text-ds-textMuted">圖表 placeholder（mock 比例）</p>
      <div className="mt-6 flex flex-col items-center gap-6 sm:flex-row sm:justify-between">
        <div
          className="flex h-44 w-44 items-center justify-center rounded-full border-4 border-dashed border-[#E5E7EB] bg-[#FAFAFF] text-center text-xs leading-snug text-ds-textSubtle"
          role="img"
          aria-label="圓餅圖預留區"
        >
          Donut
        ...
        <ul className="flex w-full max-w-xs flex-col gap-3 text-sm">
          {segments.map((s) => (
            <li key={s.label}>
              <div className="mb-1 flex justify-between text-xs font-medium text-ds-textMuted">
