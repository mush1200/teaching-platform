...
const accentMap: Record<MockReview["avatarAccent"], string> = {
  violet: "bg-[#E8E4FF] text-ds-textAccent",
  coral: "bg-[#FFE4E6] text-feedback-errorText",
  ...
  return (
    <article className={`border border-[#E5E7EB] bg-white ${compact ? "rounded-2xl p-5" : "rounded-[16px] p-5"} shadow-sm`}>
      <div className="flex flex-wrap items-center gap-2">
        ...
        {materialTitle ? (
          <span className="text-xs font-medium text-ds-textAccent">《{materialTitle}》</span>
        ) : null}
        {!compact && review.date ? (
          <time className="ml-auto text-xs text-ds-textSubtle" dateTime={review.date}>
            {review.date}
          </time>
        ) : null}
      </div>
      ...
      <div className="mt-3 flex items-center gap-2">
        ...
        {showAuthorName ? <span className="text-xs font-medium text-ds-textMuted">— {review.userName}</span> : null}
      </div>
    </article>
  );
}
