...
export default function TeacherMaterialReviewsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (role === "teacher" && target.teacher_id !== meId) {
        setError("你沒有權限查看此教材的評論。");
        setMaterial(null);
        setReviews([]);
        return;
      }
  ...
  return (
    <section className="mx-auto flex w-full max-w-4xl flex-col gap-4 px-4 py-6">
      ...
      {loading ? <LoadingState title="載入教材評論中…" /> : null}
      ...
      {!loading && !error && material ? (
        <SurfaceCard
          ...
          description={
            averageRating === null
              ? "目前尚無評論"
              : `共 ${reviews.length} 則評論，平均 ${averageRating.toFixed(1)} / 5`
          }
        >
            ...
            {reviews.length === 0 ? (
              ...
              reviews.map((review) => (
                <article key={review.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                  ...
                  <p className="mt-2 text-sm leading-relaxed text-slate-700">{review.comment?.trim() || "（無文字評論）"}</p>
                </article>
              ))
            )}
          </div>
