...
| 10 | ~~**購買者使用範圍**~~ | Terms §14.2 | ✅ **DECIDED `DEC-LEGAL-14`（2026-09-13）—— 商業意圖已鎖定，法律表述仍待律師。** 授權對象為**個別購買者／個別教師本人**（MVP **不含**機構、團隊或教師共享授權，**不得**擅自加入）。**允許**：為自己的教學改作／修改；用於**收費**教學；就其**親自任教**的學生提供**教學所必要範圍**的內容。**不允許**：轉售；公開散布；上傳至共享或公開空間；將**原始檔、可編輯檔或完整來源檔**提供給其他教師或第三人再利用。⚠️ **技術上無強制手段** —— 交付為檔案下載，無 DRM、無浮水印、無下載次數限制，因此上述界線**全部是契約限制**而非技術限制；此事實應讓律師知悉。**條文用語、可執行性與 `L-04` 的授權鏈表述維持 LAWYER REVIEW REQUIRED。** |
| 11 | ~~**授權是否專屬；平台行銷使用範圍**~~ | Creator §4.3 | ✅ **DECIDED `DEC-LEGAL-15`（2026-09-13）—— 商業意圖已鎖定，法律表述仍待律師。** 創作者授予平台之授權為**非專屬（NON-EXCLUSIVE）**；**著作權與所有權仍屬創作者**，**無任何權利移轉**。平台得為營運市集之必要而**託管、展示、技術性重製與交付**教材。**行銷素材**：`cover`／`detail`／`demo`／預覽素材得於**站內**使用，亦得於**站外**使用，用以推廣 (a) 平台本身、(b) 該創作者、(c) 該教材。**此行銷授權不包含**自由散布**完整付費教材本體**之權利。（產品現況相符：`material_media_files.kind ∈ {cover, detail, demo}` 與 `cover_image_url`／`demo_video_url` 早已於教材頁公開顯示。）**條文用語與 `L-04`／`L-05`／`L-06` 授權鏈要件維持 LAWYER REVIEW REQUIRED。** |
...
| 18 | ~~**平台停止營運計畫**~~ | Terms §13.3 | ✅ **DECIDED `DEC-LEGAL-16`（2026-09-13）—— Owner 的商業／營運「意圖」已表述，四組處置不再空白。** **計畫性停止營運**：目標**至少提前 30 日**告知（於營運上可行的前提下）。**告知期間內**：既有買家**應仍可下載已購買教材**，且平台**應明確提醒**買家於停止前自行下載備份。**創作者**應能於停止營運期間**取回／匯出**其上傳內容（於技術上可行的範圍內）。**停止營運後**：平台**不承諾**無限期託管或無限期提供下載。**撥款**：停止營運前應完成依最終撥款規則**已可確定且應支付**之創作者款項；仍在退款／爭議／回沖審查中之金額，依最終結算規則處理。⚠️ **以下維持 LAWYER REVIEW REQUIRED，Owner 意圖不得取代之**：法定最短告知期間、強制退款義務、停止營運後之存取權利、以及上述條款之可執行性。**產品現況**：目前**無任何 wind-down 機制**（entitlement 綁教材、檔案存於 B2），此意圖若要成立需要另行實作 —— 但**本輪不納入 MVP scope**。 |
...
| 20 | ~~**部署環境委外處理者揭露（事實盤點部分）**~~ | Privacy §5.4 | ✅ **事實盤點 DECIDED `DEC-LEGAL-17`（2026-09-13）；法律分類仍待律師。** ~~原記載「`PRE-01`／`PRE-02` 尚未決定」~~ —— 兩者皆已由 `DEC-13`／`DEC-16` 拍板並由 `PRE-13` 實作。**Owner 確認之現行事實清單（六家）**：(1) **Render** — 前後端主機；(2) **Neon** — production PostgreSQL；(3) **Backblaze B2** — 私有檔案 ＋ **production DB 備份儲存**；(4) **GitHub Actions** — CI ／ synthetic monitoring ／ **備份執行環境**；(5) **Healthchecks.io** — 可用性與備份監控 ping；(6) **Resend** — 交易郵件，**MVP launch 即啟用**（`DEC-18`）。**本項只確認「實際使用了哪些服務」這件事實。** ⚠️ **以下一律維持 LAWYER REVIEW REQUIRED，Owner 確認不構成任何法律認定**：受託處理者／再受託處理者之身分認定、跨境傳輸之法律分類、DPA 充分性、以及揭露之條文表述（見 `O-20`／`AD-01`／`AD-02`／`AD-07`）。**尚缺之事實**：除 Resend 已於 `O-19` 備妥完整事實表外，其餘五家之**法人名稱、DPA、分包商清單、資料所在地文件**仍未蒐集 —— 這是查找工作，不是判斷工作。 |
