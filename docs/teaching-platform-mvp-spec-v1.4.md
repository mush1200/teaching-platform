...
| PUT | `/materials/:id` | JWT (**teacher** owner or **admin**) | Update material fields. **`status` is rejected** — teacher **403**, admin **400** `{ error: "status_not_updatable_here" }`; use the review workflow endpoints below. If body includes `contents`, backend replaces existing `material_contents`; if body includes `detail_images`, backend replaces existing `material_images`. |
