...
### 6.4 ⚠️ 2026-09-13 —— 首次判定 **SUPERSEDED**（scope 變更，**非** gate 失敗）

> **§6.3 的判定文字未修改，保留為當時所作成的紀錄。** 本節是其後的狀態。

Owner 於 2026-09-13 擴大 MVP launch scope：

```text
DEC-18  交易郵件於 MVP launch 即啟用      （取代 DEC-17 的「初期不啟用」營運決定）
DEC-19  創作者撥款／分潤納入 MVP launch scope
```

**§6.3 的判定是對「當時的 scope」所作成的** —— 當時交易郵件依 `DEC-17` 不啟用、
撥款能力不存在。這兩項能力**完全沒有**出現在 §3 的任何一條判準裡，因此：

| | |
| --- | --- |
| gate 本身（§1～§5 的判準與規則） | **仍然有效，未改動** |
| §6.3 的首次判定 | **SUPERSEDED —— 不得用來描述新 scope** |
| 判定是否失敗 | **否。** 沒有發現任何技術缺陷；§6.1 的證據仍可複驗 |

**重新判定的方式：delta revalidation** —— `PRE-17`／`PRE-18` 完成後，
沿用同一份判準重跑**受影響的區塊**，並為新增的兩項能力補上判準
（郵件：寄送成功之稽核證據；撥款：金額計算正確性、退款沖銷、稽核軌跡）。
**不需要**從頭重跑整份 gate。

§5 已經要求「宣布 GO 時必須說明判定對象是哪一個 commit」。
**本節把同一條紀律延伸到 scope：判定只描述它所涵蓋的範圍。**

**Formal Launch Go 的阻擋原因因此增加一項：**
`BLOCKED ＝ Legal ＋ Domain ＋ 新納入 scope 的兩項能力`。
