...
> **【2026-09-04 Owner 決定 —— 一般客服與個資權利請求共用同一個信箱】**
>
> 本段原寫：「`/support` 上的個資權利請求段落**刻意不印出地址**……該段落顯示
> 『將於正式隱私權政策公布後提供』，由測試釘住不得洩漏草稿地址。」
> 當時的理由是：該地址只寫在**草稿**《隱私權政策》裡，四條 legal route 未發布時
> 一律 404（`TEST-01`），提前公開等於替平台做出承諾。
>
> **Owner 已明示決定改為共用同一個受監看的信箱**，該理由因此不再成立 ——
> 那個地址現在本來就以**一般客服信箱**的身分公開印在 `/support` 上。
> 繼續叫使用者等「之後才會公布的另一個地址」會是**誤導**，因為並不存在第二個信箱。
>
> **改為：** 有設定時，該段落據實說明「個資權利請求與一般客服使用同一個信箱」，
> 並請來信於**主旨註明「個資權利請求」**以利分類；未設定時維持原本誠實的等待文案。
>
> **仍然成立、且不因共用而改變的邊界：**
>
> 1. **請求類別與處理義務仍然分開** —— 個資權利請求的法律基礎（個資法）、期限來源
>    與紀錄要求都與一般客服不同（§12.11）。共用的是**收件管道**，不是**處理規則**。
> 2. **`privacy_requests` 仍是獨立 domain** —— 不得把一般客服塞進去，
>    也不得新增 `request_type` 值來承接一般問題（§12.10.1、本節下方兩條硬規則）。
> 3. **地址仍然只從環境變數來** —— `/support` 與任何 frontend 原始碼**都不得硬編**
>    該地址（由 `support-entry.spec.ts` 的 source-scan 測試釘住）。
> 4. **《隱私權政策》發布後，其所載之個資聯絡方式為準** —— 屆時若 Owner 改用
>    品牌化的 `support@<production-domain>`，只需改一個環境變數。
>
> 分流目前是**人工**的（主旨標註 ＋ 人工分類）。**本輪未新增任何後端路由、
> 分流機制或 ticket 系統** —— 那仍是 `FUT-P8`。
