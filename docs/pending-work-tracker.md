...
| | 項目 |
| --- | --- |
| **現在在做** | **`P1-09` Deployment Gate implementation（Wave 2，一次一個工作單位）** —— 最新完成 **Wave 2 #13 Gate 4 申訴證據讀取／交付** ＋ 平行 P1-09 session 的 **`H-1`**／**`DEC-02R`**（2026-08-27，見 §2.3；兩個 session 已於同日完成 unified reconciliation）。Product Readiness 的 **11 個 deployment blocker 已完成 10 個**；唯一剩下的是 `P1-09`（仍 `OPEN`）。**`P1-09` 的文件規格階段已於 2026-08-26 CLOSED**（`v1.8 Full Baseline` ＋ `FULL REGRESSION PASSED 20/20`），但**仍卡在 legal copy ＋ 14 個 Deployment Gate ＋ Legal/Tax Validation**，且**上位依賴 `PRE-03`／`PRE-04` 維持 OPEN**（見 §2.3、§6） |
...
| **Product readiness** | 稽核共 38 項；**已修 10**（`P1-01`～`P1-08`、`P1-10`、`P2-04`），**仍 open 28**（`P1-09` ＋ `P2-01`～`P2-03`、`P2-05`～`P2-16`、`P3-01`～`P3-12`）。**Deployment blocker 11 個中已關閉 10 個**；`P1-09` 是唯一剩下的 —— 見 §2.1／§2.3 |
...
> **進行中：`P1-09` DEPLOYMENT GATE IMPLEMENTATION（Wave 2）。**
>
...
> **Next Up（工程軌，2026-08-30 `DX-15` 完成後重算）：**
> ~~1. `DX-18` → 2. `DX-15`~~ **兩項皆已 ✅ DONE**（見 §1.4／§10.1）。
> **工程軌已無其他 open engineering item，因此下一項是 `OPS-05`。**
> **`OPS-05` 維持 `P3`**、**不對應 14 個 Gate 中任何一個**、**尚未開始** ——
> 它排在這裡只是因為工程軌沒有別的了，**不代表它推進 deployment readiness**。
...
| 順序 | ID | 理由 |
| --- | --- | --- |
| **0（工程軌）** | **`OPS-05`**（工程軌唯一剩下的項目） | **2026-08-30 `DX-15` 完成後重算。** `DX-15` 已 ✅ **DONE** —— root cause 是 `/` 的角色導向寫在 `app/page.tsx` 的 `useEffect` 裡而**依賴 client hydration**（實測 1.5–1.9s，再加 landing RSC 0.03–0.55s），在 N 個 worker 對單一 `next start` 的平行套件下尾端會越過 5 秒斷言預算；**產品從未壞掉**（20 秒預算的診斷 8/8 都會導向）。修法是 Owner 核准後把導向移到 **`middleware.ts`**（server-side，`matcher` 只加 `"/"`，目的地沿用 `lib/session.ts` 單一對照，產品語意零變更）。repeat matrix **0 / 40**，連續兩次完整平行套件 **DX-15 family 0 failures**，未用 timeout／retry／serial／`workers=1` 任何 workaround。**`OPS-05` 遞補 #1**：工程軌已無其他 open item。**它維持 `P3`、不對應 14 個 Gate 中任何一個、尚未開始** —— 排在這裡只是因為沒有別的了，**不代表它推進 deployment readiness**。**`PRE-03` 不列入本排序** —— 外部 critical path，與工程軌平行（法務／決策軌 #0 該列未動）。**Deployment Readiness 仍為 `0 / 14`。** |
...
| — | — | 先前的 `IA`／`COR`／`SEC` backlog 仍為 **0**；`DX` 僅剩 `DX-15`（NOT REPRODUCIBLE，opportunistic close）與 `DX-16`（deferred）。**`BUY` 已不是 0**（2026-08-28 更新）：`BUY-03`／`BUY-04` 兩項 `P1` buyer-facing false affordance 已立案，並經 Owner Decision Round 4（`DEC-09`／`DEC-10`）拍板，現為工程軌 #1／#2。`PRE-01`／`PRE-02` 維持 blocked，且 **product readiness 未完成前不啟動** |
