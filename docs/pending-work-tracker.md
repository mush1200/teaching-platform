...
> **工程軌現在的順序是：`DX-19` → `A11Y-01` → `DX-20` → `OPS-05`**
...
| 順序 | ID | 理由 |
| --- | --- | --- |
| **0（工程軌）** | **`DX-19`** | **2026-08-30 `TEST-01` 完成後重算。** 讓完整 E2E 對 live backend 的依賴變成**明示前置**，使「backend 沒開」不再偽裝成 product regression。**排第一的理由在本輪又多了一條實證：** `TEST-01` 新增的 backend contract case （斷言 404 來自「沒有 published 列」而非「後端連不上」）**正是**這一類需要 live backend 的 spec，目前全靠人工確保前置正確。**不得弱化任何既有斷言、不得新增大範圍 skip 換綠燈** —— 現有 39 個 skip 全部是既有的 viewport guard，`TEST-01` 一個都沒有增加。**不推進 Deployment Readiness**（仍 `0 / 14`）。 |
| **1** | **`A11Y-01`** | 7 處 `focus:` → `focus-visible:` 收斂，`ui-design-system.md` §10.2 已列為現況且 2026-08-30 逐檔複驗仍成立。純呈現層，**不得**擴成全站 accessibility rewrite，**不得**為此動 business logic、API 契約或權限判斷。 |
| **2** | **`DX-20`** | `.gitattributes` 缺失造成的 binary/text 判定風險（含 media-security spec 的 sub-note）。`P3`：`REL-01` 已驗證目前 blob 與 worktree byte-identical，**沒有既成損壞**，因此不急。 |
| **3** | **`OPS-05`** | 法律文件管理的 Admin UI。工程能力可做，**但真實發布仍 blocked on lawyer approval** —— `REL-01` 把 packet 納入版控**不改變**這一點（commit ≠ approval ≠ publication）。維持 `P3`，不對應 14 個 Gate 中任何一個。 |
...
| `DX-19` E2E live-backend 依賴明示化 | **OPEN — 工程軌 #0** |
| `A11Y-01` `focus-visible` 收斂 | **OPEN — 工程軌 #1** |
| `DX-20` `.gitattributes` binary/text 判定 | **OPEN — 工程軌 #2** |
| `OPS-05` 的**工程實作部分** | **OPEN — 工程軌 #3**（發布仍 blocked，見 CONDITIONAL） |
